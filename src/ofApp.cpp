...
void ofApp::update(){
    ...
    if (webcam_feed.isFrameNew()){
        ...
        // 웹캠 정보 가져와서 grayscale에 저장
        color_frame.setFromPixels(webcam_feed.getPixels());
        grayscale_frame = color_frame;                                                      // 현재 프레임을 흑백으로 가져옴
        ...
        abs_difference.threshold(threshold_val);                                            // thrsh만큼 대비값 높임
        abs_difference.dilate();                                                            // dilate --> 흐릿하게 함, 팽창, boundary 중 max 값으로 변경
